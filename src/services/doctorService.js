...
let checkRequiredField = (inputData) => {
    let arrField = ['doctorId', 'contentHTML', 'action', 'contentMarkdown', 'selectedPrice', 'selectedPayment',
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!inputId) {
                ...
                if (data && data.image) {
                    data.image = Buffer.from(data.image, 'base64').toString('binary'); // convert base64 sang binary 
